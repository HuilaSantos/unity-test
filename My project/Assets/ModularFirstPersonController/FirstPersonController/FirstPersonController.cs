    ...
    private void Update()
    {
        ...
        if(enableSprint)
        {
            ...
            else
            {
                ...
                // Freeze bar (no refill) while sprint key is held after depletion
                if(!unlimitedSprint)
                {
                    if (sprintHeld && (isSprintCooldown || sprintRemaining <= 0f))
                    {
                        // Do not refill while holding sprint after it ended
                    }
                    else
                    {
            ...
            if(isSprintCooldown)
            {
                // Only tick cooldown once sprint key is released
                if(!sprintHeld)
                {
